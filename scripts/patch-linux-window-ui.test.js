...
function currentTrayLifecycleBundleFixture() {
  return [
    ...
    "var pb=class{constructor(e){this.tray=e;if(process.platform===`linux`){this.tray.on(`click`,()=>{}),this.updatePersistentTrayMenu();return}}destroy(){this.tray.destroy()}isReady(){return typeof this.tray.isReady==`function`?this.tray.isReady():!0}async waitForReady(){if(typeof this.tray.whenReady!=`function`)return!0;try{return await this.tray.whenReady(),!0}catch{return!1}}getNativeTrayMenuItems(){return[]}updatePersistentTrayMenu(){process.platform===`linux`&&this.tray.setContextMenu(c.Menu.buildFromTemplate(this.getNativeTrayMenuItems()))}}",
  ].join("");
}
...
function latestAvatarOverlayBundleFixture() {
  return [
    ...
    "endDrag(e,t,n=`released`){let r=this.window;if(r==null||r.isDestroyed()||r.webContents.id!==e)return;let i=this.dragState,a=this.windowServerDragActive,o=null;this.dragState=null,this.windowServerDragActive=!1,this.windowServerDragWindowX=null,a?this.persistWindowBounds(r,o??this.getCurrentDisplay()):this.reclampWindowToVisibleDisplay({shouldPersist:!0});let s=this.dockTarget;s!=null&&this.dockPresentation(s.anchor,s.onDock)}",
...
test("destroys the registered Linux tray before the app exits", () => {
  ...
  assert.match(patched, /k\.once\(`closed`,\(\)=>\{globalThis\.codexLinuxPrimaryWindowClosed=!0\}\)/);
...
test("accepts stock Electron tray readiness and falls back to the Linux app icon", async () => {
  ...
  const readinessMethods = patched.match(
    /isReady\(\)\{[^}]+\}async waitForReady\(\)\{[^]*?catch\{return!1\}\}/,
  )?.[0];
  assert.ok(readinessMethods);
  const context = { result: null };
  await vm.runInNewContext(
    `let Ready=class{constructor(tray){this.tray=tray}${readinessMethods}};result=(async()=>({stockWait:await new Ready({}).waitForReady(),stockReady:new Ready({}).isReady(),nativeWait:await new Ready({whenReady:async()=>{}}).waitForReady(),nativeReady:new Ready({isReady:()=>!1}).isReady(),failedWait:await new Ready({whenReady:async()=>{throw Error(\`not ready\`)}}).waitForReady()}))()`,
    context,
  );
  assert.deepEqual(
...
test("adds a bounded will-quit drain fallback on Linux", () => {
  ...
  assert.match(
    patched,
    /codexLinuxRunQuitDrain\(\(\)=>\{c\.dispose\(\),u\.dispose\(\);return Promise\.allSettled\(\[d\.flush\(\),p\(\),m\(\)\]\)\}\)/,
  );
  assert.equal(
...
test("uses the frameless native Codex titlebar for primary Linux windows", () => {
  const source = [
    ...
    "function I2({platform:e,appearance:t,opaqueWindowSurfaceEnabled:n,prefersDarkColors:r}){return n?{backgroundColor:r?a2:o2,backgroundMaterial:e===`win32`?`none`:null}:e===`win32`&&!A2(t)?{backgroundColor:i2,backgroundMaterial:`mica`}:{backgroundColor:i2,backgroundMaterial:null}}",
...
test("adds Linux avatar overlay mouse passthrough recovery", () => {
  ...
  assert.match(patched, /this\.windowServerDragActive\|\|\(this\.windowServerDragWindowX=null\),process\.platform===`linux`&&\(this\.pointerInteractive=!0,this\.applyPointerInteractivityPolicy\(\)\)\}endDrag\([^)]*\)/);
  assert.match(patched, /this\.dockPresentation\([A-Za-z_$][\w$]*\.anchor,[A-Za-z_$][\w$]*\.onDock\),process\.platform===`linux`&&this\.applyPointerInteractivityPolicy\(\)\}setElementSize/);
...
test("adds Linux launch actions through current setSecondInstanceArgsHandler bundles", () => {
  ...
  assert.match(launchPatched, /codexLinuxPrimaryWindowNeedsColdRestart=\(\)=>\{let e=M\.getPrimaryWindow\(B\);return globalThis\.codexLinuxPrimaryWindowClosed===!0\|\|e==null\|\|e\.isDestroyed\(\)\|\|!e\.isVisible\(\)&&!e\.isMinimized\(\)\}/);
