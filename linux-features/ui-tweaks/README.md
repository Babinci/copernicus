...
| `sidebar.threadColor` | `patches/sidebar-thread-color.js` | Adds a visible color action and named color submenu to every local sidebar chat, displaying the saved color as a soft full-row tint. | `tweaks.sidebar.threadColor.enabled` |
...
Adds **Change chat color…** to the context menu for every local chat in the
sidebar, including Pinned, Projects, and Recents. The submenu offers Red, Orange, Yellow,
Green, Blue, Purple, and No color. The choice is stored in the upstream
`sidebar-thread-metadata` global-state entry for that conversation, so it
