...
function sharedTransportClassSource(symbols) {
  return (
    ...
    "socketPathIsLive(e=!1){return new Promise(t=>{let n=require(`node:net`).createConnection({path:this.socketPath}),r=!1,o=i=>{if(r)return;r=!0,clearTimeout(a),n.removeAllListeners(),n.destroy(),t(i)},a=setTimeout(()=>o(!e),500);a.unref?.(),n.once(`connect`,()=>o(!0)),n.once(`error`,t=>o(!e&&t?.code!==`ECONNREFUSED`&&t?.code!==`ENOENT`))})}" +
    ...
    "async waitForAttachedAuthority(){for(let e=0;e<50;e++){if(this.disposed)throw Error(`shared app-server socket transport is disposed`);if(await this.socketPathIsLive(!0))return;await new Promise(e=>setTimeout(e,100))}throw Error(`shared app-server attach-only socket is unavailable: ${this.socketPath}`)}" +
    "async ensureAuthority(){if(this.disposed)throw Error(`shared app-server socket transport is disposed`);if(process.env.CODEX_LINUX_APP_SERVER_BRIDGE_ATTACH_ONLY===`1`)return this.waitForAttachedAuthority();if(await this.socketPathIsLive(!0))return;if(this.authorityReady)return this.authorityReady;if(this.authority&&this.authority.exitCode==null&&this.authority.signalCode==null){if(this.authorityError)throw this.authorityError;return}let e=this.startAuthority();this.authorityReady=e;try{return await e}finally{this.authorityReady===e&&(this.authorityReady=null)}}" +
